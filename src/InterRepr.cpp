...
struct DebugArgVisitor {
  ...
  void operator()(const NoArg &noarg) { std::cout << "NoArg"; }
};
...
void debug(const Compiler &compiler) {
  for (const auto &func : compiler.functions) {
    std::cout << "Function " << func.function_name << '(' << func.num_args
              << ',' << func.max_vars_count << "):\n";
...
static std::vector<std::vector<Tokentype>> precedences = {
    {Tokentype::assignment}, // we can put plus equals,minus equals,mult
                             // equals,assignment everything here
    {Tokentype::bit_and, Tokentype::bit_or},
    {Tokentype::less, Tokentype::greater, Tokentype::equals,
...
Compiler IREmittor::EmitIR() {
  ...
  if (is_main_func_present != true)
    errorf("Main function not declared");
  compiler.data_section = datastring.str();
  return compiler;
}

void IREmittor::compile_prog() {
  if (Token attrib = try_consume(Tokentype::attribute);
      attrib || try_peek(Tokentype::function)) {
    Func func{};
    if (attrib.val == "asm")
      func.func_flags |= Flag::AsmFunc;
    ...
    while (peek().value().type == Tokentype::identifier) {
      ...
      if (try_consume(Tokentype::assignment)) {
        Token def_val = try_consume(
            Tokentype::integer_lit,
            "Only integer literals are supported as default args for now");
    ...
    if (try_consume(Tokentype::open_square)) {
      ...
      size_t val = atoll(try_consume(Tokentype::integer_lit,
                                     "Only integer sizes supported for now")
                             .val.c_str());
      try_consume(Tokentype::close_square, "Expected ]");
      compiler.arrays.emplace_back(name, val);
    } else {
      size_t val{};
      if (Token token = try_consume(Tokentype::integer_lit))
        val = atoll(token.val.c_str());
    ...
    Token func = try_consume(Tokentype::identifier,
...
void IREmittor::compile_func_body(Ops &ops) {
  ...
  if (try_consume(Tokentype::semicolon))
    ...
    return;
  else if (compile_for_loops(ops))
    return;
  else if (compile_branch(ops))
    return;
  else if (compile_asm(ops))
    return;
  else if (compile_switch(ops))
    return;
  compile_stmt(ops);
}
...
bool IREmittor::compile_switch(Ops &ops) {
  if (try_consume(Tokentype::switch_)) {
    ...
    if (try_consume(Tokentype::default_)) {
      ...
      size_t vars_size = vars.size();
      if (curr == 0)
        errorf("Default case witout any previous cases isn't allowed");
      try_consume(Tokentype::colon, "Expected :");
      ops.emplace_back(Label{labels_count});
      std::get<JmpIfZero>(ops[curr]).idx = labels_count++;
      compile_block(ops);
    ...
    for (size_t idx : backpath_indexes)
      std::get<Jmp>(ops[idx]).idx = labels_count;
    ops.emplace_back(Label{labels_count++});
  ...
  if (try_consume(Tokentype::auto_)) {
    while (peek().value().type != Tokentype::semicolon) {
      ...
      if (get_var(var_name).index != -1)
        errorf("Variable already declared {}", var_name);
...
bool IREmittor::compile_asm(Ops &ops) {
  if (try_consume(Tokentype::assembly)) {
    try_consume(Tokentype::open_paren,
                "Expected open paren after asm statement");
    std::string code =
        try_consume(Tokentype::string_lit, "Expected string lit").val;
...
Arg IREmittor::compile_prim_expr(Ops &ops) {
  ...
  if (try_peek({Tokentype::incr, Tokentype::decr})) {
    if (Var *var = std::get_if<Var>(&ret)) {
      ...
      ops.emplace_back(
          BinOp{Var{new_curr}, NoArg{}, ret, Tokentype::assignment});
      Tokentype type = consume().type;
      temp = Var{vars_count++, Storage::Auto};
      ops.emplace_back(BinOp{temp, NoArg{}, ret, Tokentype::assignment});
...
Arg IREmittor::compile_primary_expression(Ops &ops) {
  ...
  case Tokentype::identifier: {
    ...
    if (var.index == -1)
      errorf("Variable not declared {}", token.val);
    return var;
  }
  case Tokentype::integer_lit:
    return Literal{(big_int)atoll(token.val.c_str())};
  case Tokentype::string_lit: {
    std::string lit = token.val;
    for (size_t i = 0; i < lit.size(); i++) {
      if (lit[i] != '\\')
        datastring << int(lit[i]);
      else {
        i++;
        if (lit[i] == 'n')
          datastring << 10;
          ...
          assert(false && "messed up escape characters");
        i++;
      }
      datastring << ",";
    }
  ...
  case Tokentype::mult: {
    Arg arg = compile_primary_expression(ops);
    ops.emplace_back(
        BinOp{Var{vars_count}, NoArg{}, arg, Tokentype::assignment});
  ...
  case Tokentype::open_curly: {
    ...
    try_consume(Tokentype::close_curly, "Expected }");
    if (functions.find("alloc") == functions.end())
      errorf("Please provide declaration of alloc function");
    ops.emplace_back(Funcall{"alloc", {Literal{8 * args.size()}}});
    Var ptr = Var{vars_count++, Storage::Auto};
    Var temp = Var{vars_count++, Storage::Auto};
    ops.emplace_back(
        BinOp{ptr, NoArg{}, FuncResult{"alloc"}, Tokentype::assignment});
  ...
  case Tokentype::incr:
  case Tokentype::decr: {
    Token val = try_consume(Tokentype::identifier,
                            "expected identifier after pre decrement");
  ...
  case Tokentype::function: {
    ...
    try_consume(Tokentype::close_paren, "expected ')'");
    if (functions.find(funcall_name) == functions.end())
      errorf("Undeclared function {}", funcall_name);
    auto it = std::find_if(compiler.functions.begin(), compiler.functions.end(),
                           [funcall_name](const Func &func) {
                             return func.function_name == funcall_name;
                           });
    if (it != compiler.functions.end()) {
      const Func &func = *it;
      size_t num_args = func.num_args, args_size = args.size(),
             default_size = func.default_args.size();
      if (num_args > args_size && default_size >= num_args - args_size) {
        for (size_t idx = args_size; idx < num_args; idx++)
          args.emplace_back(Literal{func.default_args[idx - args_size]});
      } else if (num_args != args_size)
        errorf("Function signature for {} not matched . Expected atleast {} "
...
Token IREmittor::consume() { return tokens[token_index++]; }
